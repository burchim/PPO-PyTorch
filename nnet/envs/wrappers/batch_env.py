...
class BatchEnv:
    ...
    def __init__(self, envs, parallel=False):
        ...
        self.tuple_state = self.num_states > 1
        ...
        if self.parallel:
            ...
            # Void Obs
            state = tuple([torch.zeros((self.num_envs,) + obs_space[1], dtype=obs_space[2]) for obs_space in self.envs[0].obs_space()])

            # Create Buffer
            self.buffer = structs.AttrDict(
                state=state if len(state)>1 else state[0],
